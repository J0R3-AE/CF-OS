...
void atomic_add(volatile u32 *ptr, u32 inc){(void)atomic_fetch_add(ptr, inc);} 
void atomic_sub(volatile u32 *ptr, u32 dec){(void)atomic_fetch_sub(ptr, dec);}
u32 atomic_load(volatile u32 *ptr){return __atomic_load_n(ptr, __ATOMIC_ACQUIRE);}
void atomic_store(volatile u32 *ptr, u32 val){__atomic_store_n(ptr, val, __ATOMIC_RELEASE);}
void atomic_cpu_pause(void){__asm__ volatile("pause");}