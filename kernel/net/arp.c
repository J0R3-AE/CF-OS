...
int net_arp_insert(const u8 ip[4], const u8 mac[NET_MAC_LEN])
{
    ...
    for (usize i = 0; i < NET_ARP_CACHE_SIZE; ++i)
    {
        if (g_arp_cache[i].valid &&
            memcmp(g_arp_cache[i].ip, ip, sizeof(g_arp_cache[i].ip)) == 0)
        {
...
{
    if (!dev || !frame || len < sizeof(net_ethernet_header_t) + sizeof(net_arp_header_t))
        return NET_EINVAL;

    const net_ethernet_header_t *eth = frame;
    if (net_ethernet_type(eth) != NET_ETHERTYPE_ARP)
        return NET_EINVAL;
