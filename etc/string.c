...
usize strlen(const char *s)
{
    ...
    while ((uintptr_t)p % sizeof(uintptr_t) != 0)
    {
        if (!*p)
            return (usize)(p - s);
        ++p;
    }
    // Now aligned, check word by word
    const uintptr_t *wp = (const uintptr_t *)p;
    while (1)
    {
        uintptr_t w = *wp++;
        if ((w - (uintptr_t)0x0101010101010101ULL) & ~(w) & (uintptr_t)0x8080808080808080ULL)
        {
            // Found null byte in this word
            p = (const char *)(wp - 1);
            while (*p)
                ++p;
            return (usize)(p - s);
