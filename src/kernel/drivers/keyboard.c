...
static int translate_scancode(uint8_t code)
{
    ...
    /* special keys - return ASCII equivalents */
    case 0x1C:
        return '\n';  /* enter = newline ASCII */
    case 0x01:
        return 27;    /* ESC ASCII */
...
static void handle_scancode(uint8_t scancode)
{
    ...
    /* extended keys - just skip for now (arrow keys need special handling) */
    if (extended)
    {
        extended = false;
        /* Skip extended keys like arrow keys - they need special handling */
        return;
    }
