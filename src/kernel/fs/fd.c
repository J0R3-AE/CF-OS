...
int fd_alloc_at(fd_table_t *t, int want_fd, struct file *f)
{
    ...
    if (want_fd < 0 || want_fd >= FD_MAX) return EINVAL;
...
struct file *fd_get(fd_table_t *t, int fd)
{
    ...
    if (!t->entries[fd].used) return NULL;
...
int fd_close(fd_table_t *t, int fd)
{
    ...
    if (!t->entries[fd].used) return EINVAL;
    ...
    file_unref(f); // release the fd's reference
